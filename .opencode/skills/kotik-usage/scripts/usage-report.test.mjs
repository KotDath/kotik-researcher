...
test('Kimi K3 uses Moonshot list pricing without losing reported cost', () => {
  ...
  const priced = priceUsage(usage, 'kimi-for-coding', 'k3')
  ...
  assert.equal(priceUsage(usage, 'other', 'k3').cost, 0)
})
...
test('GPT-5.6 applies long-context multipliers per message', () => {
  ...
  const priced = priceUsage(usage, 'openai', 'gpt-5.6-sol')
...
test('explore inherits its parent stage except under orchestrator', () => {
  const sessions = new Map([
    ['root', { id: 'root', parent_id: null, agent: 'orchestrator' }],
    ['review', { id: 'review', parent_id: 'root', agent: 'reviewer' }],
    ['nested', { id: 'nested', parent_id: 'review', agent: 'explore' }],
    ['direct', { id: 'direct', parent_id: 'root', agent: 'explore' }]
  ])
  assert.equal(stageForAgent('explore', sessions.get('nested'), sessions), 'review')
  assert.equal(stageForAgent('explore', sessions.get('direct'), sessions), 'exploration')
  assert.equal(stageForAgent('spec-writer', null, sessions), 'specification')
})

test('current root report aggregates roles, stages and multiple models', () => {
  ...
  try {
    ...
    assert.equal(report.byStage.get('review').total, 49)
    assert.equal(report.byStage.get('exploration').total, 9)
    assert.equal(report.byStage.get('specification').total, 13)
    assert.equal(report.byStage.get('other:custom-agent').total, 6)
    assert.equal(report.byModel.get('fixture/flash').messages, 4)
    assert.equal(report.byModel.get('fixture/pro').messages, 3)
    assert.equal(report.byRoleModel.get('implementer\u0000fixture/flash').total, 68)
    assert.equal(report.byRoleModel.get('implementer\u0000fixture/pro').total, 5)
    assert.equal(report.bySession.get('impl').models.size, 2)

    const markdown = renderReport(report)
    assert.match(markdown, /## По ролям и моделям/)
    assert.match(markdown, /Cache miss, токены \| Cache miss, цена/)
    ...
    assert.match(markdown, /## Этап × модель/)
...
test('--days filters messages by timestamp across root trees', () => {
  ...
  try {
    ...
    assert.equal(report.total.total, 186)
    assert.equal(report.rootCount, 1)
    assert.equal(report.byStage.get('coordination').total, 12)
