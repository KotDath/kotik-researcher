...
//
// Калибровка фраз коррекций — под пользователя (см. docs/LESSONS.md, практика Yu):
...
const CORRECTION_WORDS = ["нет", "нет,", "no", "stop"];
...
function parseArgs(argv) {
  const args = { days: null, session: null, from: null, to: null, budget: TOTAL_BUDGET, positional: null };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--help" || a === "-h") { console.log("Usage: session-digest.mjs [sessionID] [--days N] [--session ID --from MSG --to MSG] [--budget BYTES]"); process.exit(0); }
...
const correctionRe = CORRECTION_PHRASES.map(p => new RegExp(p.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
const wordRe = CORRECTION_WORDS.map(w => new RegExp("(^|[^\\p{L}])" + w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "([^\\p{L}]|$)", "iu"));
function isCorrection(text) {
  if (!text) return false;
  return correctionRe.some(r => r.test(text)) || wordRe.some(r => r.test(text));
}

function openDb() {
  try { return new DatabaseSync(DB_PATH, { readOnly: true }); }
  catch (e) { console.error("Не могу открыть БД " + DB_PATH + ": " + e.message); process.exit(1); }
}
...
function analyze(sess) {
  ...
  for (const u of userTexts) {
    ...
    if (!isCorrection(u.text)) continue;
  ...
  const endsOnCorrection = userTexts.length > 0 && isCorrection(userTexts[userTexts.length - 1].text)
...
function main() {
  ...
  for (const rootId of rootIds) {
    for (const id of treeIds(db, rootId)) {
      ...
      if (!sess) continue;
      totalSessions++;
      sections.push(renderSession(analyze(sess), SESSION_CAP));
    }
  }
