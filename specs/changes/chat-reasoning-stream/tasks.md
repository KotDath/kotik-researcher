...
- [ ] 8.1 `pnpm typecheck` и `pnpm lint` проходят
- [ ] 8.2 Smoke: `pnpm build` собирается, собранный артефакт запускается
- [ ] 8.3 Сценарии дельты pi-chat-sessions: reasoning стримится перед текстом
...
- [ ] 8.4 Сценарии персистентности: после перезапуска блоки из истории свёрнуты,
  с той же длительностью; оборванный reasoning в истории отсутствует; после
  auto-retry в ленте один блок reasoning без дублей
- [ ] 8.5 Сценарии дельты llm-provider-settings: уровень per-provider
...
- [ ] 8.6 Модель без поддержки thinking: чат без reasoning-блоков,
