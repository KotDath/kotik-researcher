...
- [ ] 5.1 Скриншот главного окна приложения (пустой проект / dashboard)
- [ ] 5.2 Скриншот чата с историей сообщений (создать тестовый проект с предзаполненной сессией)
- [ ] 5.3 Скриншот экрана настроек (Settings)
- [ ] 5.4 Скриншот модального окна (например, создание проекта)
- [ ] 5.5 Скриншот состояния загрузки (loading spinner / skeleton)
- [ ] 5.6 Скриншот состояния ошибки (карточка ошибки в чате)
- [ ] 5.7 Скриншот пустого состояния (нет чатов / нет проектов)
- [ ] 5.8 Для каждого скриншотного теста: замаскировать динамические регионы (дата/время, аватары), использовать `maxDiffPixelRatio: 0.001`, `animations: 'disabled'`, `caret: 'hide'`
- [ ] 5.9 Создать baseline: `pnpm exec playwright test --project=visual --update-snapshots`, закоммитить скриншоты в `tests/visual/__screenshots__/`
- [ ] 5.10 Убедиться, что `pnpm test:visual` проходит без отличий от baseline
